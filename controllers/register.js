...
async function register(req, res) {
	try {
		...
		// console.log(registerModel,"ppppppppppppppppppp");
		...
		const registration = await registerModel(
			firstName,
			lastName,
			email,
			password,
		...
		return res
			.json({ success: false, message: 'Error while registering.' })
			.status(500);
	}
}
...
async function login(req, res) {
	...
	try {
		...
		if (user) {
			const tokens = await jwt.sign({ email: user.email }, 'secretkey', {
				expiresIn: '5000',
			});
